define(['gen', 'bat', 'shared'], function(gen, bat, shared){
    ...
    return {
        ...
        'tests': [
            ["Simple TakeWhile",
            function(){
                var g = bat.takeWhile(gen(shared.count()), function(v){ return v < 2; });
                assert.equal(g(), 0);
                assert.equal(g(), 1);
                assert.throws(g);
            }],
            ["Zero TakeWhile",
            function(){
                var g = bat.takeWhile(gen(shared.count()), function(v){ return false; });
                assert.throws(g);
            }],
            ["Length TakeWhile",
            function(){
                var g = bat.take(gen(shared.count(2)), function(v){ return v < 4; });
                assert.equal(g(), 0);
                assert.equal(g(), 1);
                assert.throws(g);
            }], 
