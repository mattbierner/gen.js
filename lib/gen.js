...
var compose = (function(){
    ...
    return function(f /*, ...funcs*/) {
        if (arguments.length <= 1) {
            return f;
        } else {
            var funcs = arguments;
            return function(/*...*/) { return reduceRight.call(funcs, reduce, arguments)[0]; };
        }
    };
}());
 ...
 *     source.
 * 
 * @return {function(yield, break)} A simple generator function that performs
 ...
 * 
 * @return {function(yield, break)} Simple generator function that reduces the
 ...
 * 
 */
var YieldBreak = function(){};
YieldBreak.prototype = new Error();
YieldBreak.prototype.name = "YieldBreak";
...
var gen = (function(){
    ...
    var defaultB = constant(function() { throw new YieldBreak(); });
    ...
    return function factory(source) {
        ...
        return Object.defineProperties(generator, {
            'map': {'value': compose(factory, bind(map, generator)) },
            'filter': {'value': compose(factory, bind(filter, generator)) },
            'reduce': {'value': compose(factory, bind(reduce, generator)) },
            'toArray': {'value': compose(factory, bind(toArray, generator)) },
        });
    };
}());
...
return Object.defineProperties(gen, {
    'map': {'value': compose(gen, function(s, c, t){ return map(gen(s), c, t); })},
    'filter': {'value': compose(gen, function(s, p, t){ return filter(gen(s), p, t); }) },
    'reduce': {'value': compose(gen, function(s, c, i){ return reduce(gen(s), c, i); }) },
    'toArray': {'value': compose(gen, function(s){ return toArray(gen(s)); }) }
});
