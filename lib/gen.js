...
var forEach = function(source, callback, t) {
    var i = 0;
    return single(source.sync(function(v) {
        callback.call(t, v, i++);
    }, noop));
};
...
var map = function(source, callback, thisObj) {
    source = source.sync(function(v) {
        return constant(callback.call(thisObj, v));
    });
    return gen(function(y, b) {
        return y(source(undefined, b));
    });
};
...
var prototype = {
    ...
    'filter': function(p, t) { return filter(this.sync(), p, t); },
...
gen = (function(){
    ...
    var next = function(impl, defaultY, defaultB) {
        defaultY = defaultY || globalY;
        defaultB = defaultB || globalB;
        return function(y, b) {
            return impl(y || defaultY, b || defaultB);
        }
    }
    ...
    return function(source) {
        return Object.create(prototype, {
            'sync': {
                'value': next.bind(undefined, syncNext.bind(undefined, source))
            },
            'async': {
                'value': next.bind(undefined, asyncNext.bind(undefined, source))
            }
        });
    };
}());
